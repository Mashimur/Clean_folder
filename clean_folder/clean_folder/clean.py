        ...
        if archive.name == "archives" and len(list(archive.iterdir())) != 0:
            for arch in archive.iterdir():
                if arch.is_file() and arch.suffix in (".zip", ".gz", ".tar"):
                    ...
                    continue
        elif archive.is_dir() and not len(list(archive.iterdir())):
...
def main():
    try:
        ...
        print("\nНе вказано шлях! Повторіть команду з правильними параметрами!\n")
        return
    ...
    try:
        ...
        print("\nДана папка не існує! Перевірте вказаний шлях та повторіть команду!\n")
        return
